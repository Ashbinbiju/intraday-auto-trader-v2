...
## 2. Real-Time Core ⚡
- **WebSocket Orders**: Instant fill notifications via `smart_websocket.py`.
- **Postback**: Backup HTTPS webhook at `/webhook/angel-one`.
- **Persistence Layer**: `state_manager.py` saves `BOT_STATE` to disk (`bot_state.json`) periodically and on every critical update. Prevents data loss during restarts.
- **Self-Healing Reconciliation**: On startup and every minute, the bot syncs with Angel One. It imports "Orphan" trades (manual/external buys) and auto-manages them with your default SL/Target settings!
- **Late Entry Protection**: The Strategy now rejects buy signals if the price is > 1.5% away from the EMA 20, preventing "FOMO" entries at the top.
...
- `.`: Backend (Python)
    ...
    -   `main.py`: Trading Logic (runs in background).
...
Currently, `DRY_RUN = True` in `main.py`. To enable real money execution:
1.  Edit `main.py`.
2.  Set `DRY_RUN = False`.
