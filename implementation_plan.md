...
## Current Problems
1. **Fixed 1% SL** → Too tight for intraday, gets hunted by normal pullbacks
2. **Fixed 2% TP** → Random number, not aligned with actual resistance levels
3. **No structure validation** → Trades placed regardless of market context

## Proposed Solution

### Phase 1: Structure-Based Stop-Loss ⚡ (High Priority)

#### 1.1 Calculate Dynamic SL
**Location**: `main.py` - Buy order section (before placing order)
...
- **File**: `main.py`, lines 555-590 (buy order section)
- **Change**: Before calling `place_buy_order()`, calculate structure-based SL
- **Validation**: If SL is invalid, skip the trade entirely

---

### Phase 2: Structure-Based Take-Profit 🎯

#### 2.1 Calculate Dynamic TP
**Location**: `indicators.py` (new function)
...
- Option B: Store PDH in `bot_state.json` daily
- Recommended: **Option B** (simpler, faster)

---

### Phase 3: Enhanced Trailing Logic 🏃

#### Already Implemented (Verify):
- Breakeven lock after +1R (line 220-231 in `main.py`)
...
if pos.get('is_breakeven_active'):
    ...
    df = fetch_candle_data(smartApi, token, symbol, "FIVE_MINUTE", days=1)
    if df is not None and len(df) >= 2:
        df = calculate_indicators(df)
        last_candle = df.iloc[-2]  # Confirmed candle (not current)
        last_close = last_candle['close']
        current_vwap = last_candle['VWAP']
        
        # If CANDLE CLOSED below VWAP, exit (not just LTP wick)
        if last_close < current_vwap:
            logger.info(f"📉 VWAP Trail Exit: {symbol} (Close: {last_close} < VWAP: {current_vwap})")
            place_sell_order(...)
```
...
- [ ] Create `calculate_structure_based_sl()` in `main.py`
- [ ] Create `calculate_structure_based_tp()` in `indicators.py`
...
Add new section to `config.json`:
