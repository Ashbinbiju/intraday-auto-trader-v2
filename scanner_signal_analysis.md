...
- Entry on 14:30-14:45 15M candle might have been:
  ...
  - Result: ✅ PASS (Green candle above VWAP/EMA)
...
The `check_buy_condition()` function has solid logic:
```python
if price <= vwap:
...
```
14:52:04 - ✅ Structure Risk Validated: WHIRLPOOL
14:52:04 - SL: ₹772.45 | Swing Low (0.92%)
14:52:04 - TP: ₹790.32 | 1.5R Minimum (R:R 1.5)
```
...
if latest['close'] < latest['VWAP']:
    logger.warning(f"❌ ENTRY REJECTED: {symbol} | Current price below VWAP")
    continue

if latest['close'] < latest['EMA_20']:
    logger.warning(f"❌ ENTRY REJECTED: {symbol} | Current price below EMA20")
    continue
```

### 2. **Use 5-Minute Candles for Scanner** (Alternative)

Change scanner timeframe from 15M → 5M for faster signals:
...
Current flow:
```
Scanner runs → 25s delay → Signals generated → Order placed
```
...
This is the safest, easiest fix. I can add this to `main.py` right after the scanner generates signals but before calling `place_buy_order()`.
...
**WHIRLPOOL:** 
- ❌ Bad entry (downtrend on 5M)
- ⚠️ Might have been valid on 15M 7 minutes ago
- 🎲 Outcome depends on luck (could bounce or continue falling)

**LATENTVIEW:**
- ⚠️ Marginal entry (choppy consolidation)
